...
const EditFeature = (props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const feature_detail = { id: values?.id, name: values?.name };
        await axios.put("http://localhost:3000/api/features", feature_detail);
        resetForm();
...
const handleClose = () =>{
  ...
  useEffect(() => {
    const data = localStorage.getItem("feature_data");
    if (data) {
